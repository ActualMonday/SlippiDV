...
// Compute total stock damage taken (I actually think this may be bugged... I'm seeing a lot of null values)
...
parentPort.on('message', async (filePath) => {
    try {
        ...
        if (!metadata || settings.players.length > 2) {
            ...
            //parentPort.postMessage({ doubles: true, filePath });
            parentPort.postMessage({ rejected: "doubles", filePath });
            return;
        }

        //reject for offline or invalid slippiID
        ...
        if (slippiID === settings.players[0].connectCode) {
            ...
            //parentPort.postMessage({ rejected: true, filePath });
            //parentPort.postMessage({ invalidid: true, filePath });
            parentPort.postMessage({ rejected: "invalidid", filePath });
            return;
        }

        //reject for quick quit out
        const stats = game.getStats();
        const matchTime = stats.playableFrameCount / 60;
        if (matchTime < 20) {
            //parentPort.postMessage({ rejected: true, filePath });
            //parentPort.postMessage({ quitout: true, filePath });
            parentPort.postMessage({ rejected: "quitout", filePath });
            return;
        }
